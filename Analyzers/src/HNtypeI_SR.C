...
void HNtypeI_SR::initializeAnalyzer(){
  ...
  FakeRateIDs = {"HNtypeI_16", "HNtypeI_16", "HNtypeI_16"}; //JH : to see high pt efficiency
    ...
    if(!IsDATA){
      if(DataYear==2016){
        if(it_ch==0){
          ...
          trigger_lumi = ev.GetTriggerLumi("Full"); //JH : to see how many events are cut with high pt
        }
