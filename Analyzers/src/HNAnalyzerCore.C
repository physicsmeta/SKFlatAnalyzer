...
void HNAnalyzerCore::SwitchToTempDir(){
  ...
  while (!tempDir) {
    ...
    dirname << "HNAnalyzerCore" << counter;
...
void HNAnalyzerCore::SetOutfilePath(TString outname){
  outfile = new TFile(outname,"RECREATE");
};

Event HNAnalyzerCore::GetEvent(){
...
std::vector<Muon> HNAnalyzerCore::GetAllMuons(){
  ...
  for(unsigned int i=0; i<muon_pt->size(); i++){
    ...
    //==== TuneP
    //==== Apply scailing later with HNAnalyzerCore::UseTunePMuon()
...
std::vector<Muon> HNAnalyzerCore::GetMuons(TString id, double ptmin, double fetamax){
...
std::vector<Electron> HNAnalyzerCore::GetAllElectrons(){
...
std::vector<Electron> HNAnalyzerCore::GetElectrons(TString id, double ptmin, double fetamax){
...
std::vector<Lepton *> HNAnalyzerCore::MakeLeptonPointerVector(const std::vector<Muon>& muons, double TightIso, bool UseMini){

  std::vector<Lepton *> out;
  for(unsigned int i=0; i<muons.size(); i++){
    Lepton *l = (Lepton *)(&muons.at(i));
    if( !(l->LeptonFlavour() == Lepton::MUON) ){
//      cout << "[HNAnalyzerCore::MakeLeptonPointerVector(std::vector<Muon>& muons)] Not muon.." << endl;
      cerr << "[HNAnalyzerCore::MakeLeptonPointerVector(std::vector<Muon>& muons)] Not muon.." << endl;
      exit(EXIT_FAILURE);
    }
    if(TightIso>0){
...
std::vector<Lepton *> HNAnalyzerCore::MakeLeptonPointerVector(const std::vector<Electron>& electrons, double TightIso, bool UseMini){

  std::vector<Lepton *> out;
  for(unsigned int i=0; i<electrons.size(); i++){
    Lepton *l = (Lepton *)(&electrons.at(i));
    if( !(l->LeptonFlavour() == Lepton::ELECTRON) ){
//      cout << "[HNAnalyzerCore::MakeLeptonPointerVector(std::vector<ELECTRON>& electrons)] Not electron.." << endl;
      cerr << "[HNAnalyzerCore::MakeLeptonPointerVector(std::vector<ELECTRON>& electrons)] Not electron.." << endl;
      exit(EXIT_FAILURE);
    }
    if(TightIso>0){
...
std::vector<Photon> HNAnalyzerCore::GetAllPhotons(){
...
std::vector<Photon> HNAnalyzerCore::GetPhotons(TString id, double ptmin, double fetamax){
...
std::vector<Jet> HNAnalyzerCore::GetAllJets(){
...
std::vector<Jet> HNAnalyzerCore::GetJets(TString id, double ptmin, double fetamax){
...
std::vector<FatJet> HNAnalyzerCore::GetAllFatJets(){
...
std::vector<FatJet> HNAnalyzerCore::GetFatJets(TString id, double ptmin, double fetamax){
...
std::vector<Gen> HNAnalyzerCore::GetGens(){
...
std::vector<LHE> HNAnalyzerCore::GetLHEs(){
...
std::vector<Muon> HNAnalyzerCore::UseTunePMuon(const std::vector<Muon>& muons){
...
std::vector<Muon> HNAnalyzerCore::SelectMuons(const std::vector<Muon>& muons, TString id, double ptmin, double fetamax){
...
std::vector<Electron> HNAnalyzerCore::SelectElectrons(const std::vector<Electron>& electrons, TString id, double ptmin, double fetamax){
...
std::vector<Jet> HNAnalyzerCore::SelectJets(const std::vector<Jet>& jets, TString id, double ptmin, double fetamax){
...
std::vector<FatJet> HNAnalyzerCore::SelectFatJets(const std::vector<FatJet>& jets, TString id, double ptmin, double fetamax){
...
std::vector<Electron> HNAnalyzerCore::ScaleElectrons(const std::vector<Electron>& electrons, int sys){
...
std::vector<Electron> HNAnalyzerCore::SmearElectrons(const std::vector<Electron>& electrons, int sys){
...
std::vector<Muon> HNAnalyzerCore::ScaleMuons(const std::vector<Muon>& muons, int sys){
  ...
  for(unsigned int i=0; i<muons.size(); i++){
    ...
    //==== Even for TuneP muons, MomentumShift() are set correctly from HNAnalyzerCore::UseTunePMuon()
...
std::vector<Jet> HNAnalyzerCore::ScaleJets(const std::vector<Jet>& jets, int sys){
...
std::vector<Jet> HNAnalyzerCore::SmearJets(const std::vector<Jet>& jets, int sys){
...
std::vector<FatJet> HNAnalyzerCore::ScaleFatJets(const std::vector<FatJet>& jets, int sys){
...
std::vector<FatJet> HNAnalyzerCore::SmearFatJets(const std::vector<FatJet>& jets, int sys){
...
std::vector<FatJet> HNAnalyzerCore::ScaleSDMassFatJets(const std::vector<FatJet>& jets, int sys){
...
std::vector<FatJet> HNAnalyzerCore::SmearSDMassFatJets(const std::vector<FatJet>& jets, int sys){
...
bool HNAnalyzerCore::PassMETFilter(){
...
void HNAnalyzerCore::initializeAnalyzerTools(){
...
double HNAnalyzerCore::GetPrefireWeight(int sys){
...
//  cout << "[HNAnalyzerCore::GetPrefireWeight] wtf" << endl;
  cerr << "[HNAnalyzerCore::GetPrefireWeight] wtf" << endl;
  exit(EXIT_FAILURE);
  return 1.;

}

double HNAnalyzerCore::GetPileUpWeight(int N_pileup, int syst){
...
//      cout << "[HNAnalyzerCore::GetPileUpWeight] Wrong year : " << DataYear << endl;
      cerr << "[HNAnalyzerCore::GetPileUpWeight] Wrong year : " << DataYear << endl;
      exit(EXIT_FAILURE);
      return 1.;
    }
...
double HNAnalyzerCore::GetPDFReweight(){

  return GetPDFWeight(pdfReweight->NewPDF)/GetPDFWeight(pdfReweight->ProdPDF);

}

double HNAnalyzerCore::GetPDFReweight(int member){

  return GetPDFWeight(pdfReweight->PDFErrorSet.at(member))/GetPDFWeight(pdfReweight->ProdPDF);

}

bool HNAnalyzerCore::IsOnZ(double m, double width){
  if( fabs(m-M_Z) < width ) return true;
  else return false;
}

double HNAnalyzerCore::MT(TLorentzVector a, TLorentzVector b){
  double dphi = a.DeltaPhi(b);
  return TMath::Sqrt( 2.*a.Pt()*b.Pt()*(1.- TMath::Cos(dphi) ) );
}

double HNAnalyzerCore::MT2(TLorentzVector a, TLorentzVector b, Particle METv, double METgap){
...
double HNAnalyzerCore::projectedMET(TLorentzVector a, TLorentzVector b, Particle METv){
...
bool HNAnalyzerCore::HasFlag(TString flag){

  //cout << "[HNAnalyzerCore::HasFlag] Userflags.size() = " << Userflags.size() << endl;
  //for(unsigned int i=0; i<Userflags.size(); i++){
  //  cout << "[HNAnalyzerCore::HasFlag] " << Userflags.at(i) << endl;
  //}
...
std::vector<Muon> HNAnalyzerCore::MuonWithoutGap(const std::vector<Muon>& muons){
...
std::vector<Muon> HNAnalyzerCore::MuonPromptOnly(const std::vector<Muon>& muons, const std::vector<Gen>& gens){
...
std::vector<Muon> HNAnalyzerCore::MuonPromptOnlyHNtypeI(const std::vector<Muon>& muons, const std::vector<Gen>& gens){
  ...
  for(unsigned int i=0; i<muons.size(); i++){
    if(GetLeptonType(muons.at(i), gens)<=0 && GetLeptonType(muons.at(i), gens)>-5) continue;
    out.push_back( muons.at(i) );
  }
...
Muon HNAnalyzerCore::MuonUsePtCone(const Muon& muon){
...
std::vector<Muon> HNAnalyzerCore::MuonApplyPtCut(const std::vector<Muon>& muons, double ptcut){
...
std::vector<Electron> HNAnalyzerCore::ElectronPromptOnly(const std::vector<Electron>& electrons, const std::vector<Gen>& gens){
...
std::vector<Electron> HNAnalyzerCore::ElectronPromptOnlyHNtypeI(const std::vector<Electron>& electrons, const std::vector<Gen>& gens){
...
std::vector<Electron> HNAnalyzerCore::ElectronUsePtCone(const std::vector<Electron>& electrons){
...
Electron HNAnalyzerCore::ElectronUsePtCone(const Electron& electron){
...
std::vector<Electron> HNAnalyzerCore::ElectronApplyPtCut(const std::vector<Electron>& electrons, double ptcut){
...
std::vector<Jet> HNAnalyzerCore::JetsAwayFromFatJet(const std::vector<Jet>& jets, const std::vector<FatJet>& fatjets, double mindr){
...
std::vector<Jet> HNAnalyzerCore::JetsInsideFatJet(const std::vector<Jet>& jets, const std::vector<FatJet>& fatjets, double mindr){
  ...
  for(unsigned int i=0; i<jets.size(); i++){
    ...
    if(!Outside) out.push_back( jets.at(i) );
...
std::vector<Jet> HNAnalyzerCore::JetsVetoLeptonInside(const std::vector<Jet>& jets, const std::vector<Electron>& els, const std::vector<Muon>& mus, double dR){
...
std::vector<FatJet> HNAnalyzerCore::FatJetsVetoLeptonInside(const std::vector<FatJet>& jets, const std::vector<Electron>& els, const std::vector<Muon>& mus, double dR){
...
std::vector<Jet> HNAnalyzerCore::JetsAwayFromPhoton(const std::vector<Jet>& jets, const std::vector<Photon>& photons, double mindr){
...
Particle HNAnalyzerCore::AddFatJetAndLepton(const FatJet& fatjet, const Lepton& lep){
...
std::vector<Jet> HNAnalyzerCore::JetsAwayFromLepton(const std::vector<Jet>& jets, const Muon& muon, double mindphi){
  ...
  for(unsigned int i=0; i<jets.size(); i++){
    ...
    if(( jets.at(i) ).DeltaPhi( this_muon ) > mindphi ) out.push_back( jets.at(i) );
...
std::vector<Jet> HNAnalyzerCore::JetsAwayFromLepton(const std::vector<Jet>& jets, const Electron& electron, double mindphi){
  ...
  for(unsigned int i=0; i<jets.size(); i++){
    ...
    if(( jets.at(i) ).DeltaPhi( this_electron ) > mindphi ) out.push_back( jets.at(i) );
...
Particle HNAnalyzerCore::UpdateMETMuon(const Particle& METv, const std::vector<Muon>& muons){
...
Particle HNAnalyzerCore::UpdateMETElectron(const Particle& METv, const std::vector<Electron>& electrons){
...
Particle HNAnalyzerCore::UpdateMETElectronCF(const Particle& METv, const std::vector<Electron>& electrons1, const std::vector<Electron>& electrons2){
...
std::vector<Electron> HNAnalyzerCore::ShiftElectronEnergy(const std::vector<Electron>& electrons, AnalyzerParameter param, bool applyshift){
...
void HNAnalyzerCore::PrintGen(const std::vector<Gen>& gens){
...
Gen HNAnalyzerCore::GetGenMatchedLepton(const Lepton& lep, const std::vector<Gen>& gens){
...
//    cout << "[HNAnalyzerCore::GetGenMatchedLepton] input lepton flavour not set" << endl;
    cerr << "[HNAnalyzerCore::GetGenMatchedLepton] input lepton flavour not set" << endl;
    exit(EXIT_FAILURE);
  }
...
Gen HNAnalyzerCore::GetGenMatchedPhoton(const Lepton& lep, const std::vector<Gen>& gens){
...
vector<int> HNAnalyzerCore::TrackGenSelfHistory(const Gen& me, const std::vector<Gen>& gens){
...
bool HNAnalyzerCore::IsFromHadron(const Gen& me, const std::vector<Gen>& gens){
...
int HNAnalyzerCore::GetLeptonType(const Lepton& lep, const std::vector<Gen>& gens){
  ...
  //cout << "[HNAnalyzerCore::GetLeptonType] [Reco] pt = " << lep.Pt() << "\t, eta = " << lep.Eta() << endl;
  //cout << "[HNAnalyzerCore::GetLeptonType] [Gen] Index = " << gen_closest.Index() << endl;
...
*/  
  ...
  //==== Has macthed gen lepton
    ...
    //cout << "[HNAnalyzerCore::GetLeptonType] Start Index = " << Start.Index() << endl;
    //cout << "[HNAnalyzerCore::GetLeptonType] MotherOf_Start Index = " << MotherOf_Start.Index() << endl;
...
int HNAnalyzerCore::GetGenPhotonType(const Gen& genph, const std::vector<Gen>& gens){
...
bool HNAnalyzerCore::IsSignalPID(int pid){
...
TH1D* HNAnalyzerCore::GetHist1D(TString histname){
...
TH2D* HNAnalyzerCore::GetHist2D(TString histname){
...
TH3D* HNAnalyzerCore::GetHist3D(TString histname){
...
void HNAnalyzerCore::FillHist(TString histname, double value, double weight, int n_bin, double x_min, double x_max){
...
void HNAnalyzerCore::FillHist(TString histname, double value, double weight, int n_bin, double *xbins){
...
void HNAnalyzerCore::FillHist(TString histname,
...
TH1D* HNAnalyzerCore::JSGetHist1D(TString suffix, TString histname){
...
void HNAnalyzerCore::JSFillHist(TString suffix, TString histname, double value, double weight, int n_bin, double x_min, double x_max){
...
TH2D* HNAnalyzerCore::JSGetHist2D(TString suffix, TString histname){
...
void HNAnalyzerCore::JSFillHist(TString suffix, TString histname,
                  double value_x, double value_y,
                  double weight,
                  int n_binx, double x_min, double x_max,
                  int n_biny, double y_min, double y_max){
...
void HNAnalyzerCore::FillLeptonPlots(std::vector<Lepton *> leps, TString this_region, double weight){
...
//      cout << "[HNAnalyzerCore::FillLeptonPlots] lepton flavour wrong.." << endl;
      cerr << "[HNAnalyzerCore::FillLeptonPlots] lepton flavour wrong.." << endl;
      exit(EXIT_FAILURE);
    }
...
void HNAnalyzerCore::FillJetPlots(std::vector<Jet> jets, std::vector<FatJet> fatjets, TString this_region, double weight){
