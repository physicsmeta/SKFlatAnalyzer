#include "HNtypeI_WZ_CR.h"

HNtypeI_WZ_CR::HNtypeI_WZ_CR(){

}

void HNtypeI_WZ_CR::initializeAnalyzer(){

  //==== if you use "--userflags RunSyst" with SKFlat.py, HasFlag("RunSyst") will return "true"
//  RunSyst = HasFlag("RunSyst");
//  cout << "[HNtypeI_WZ_CR::initializeAnalyzer] RunSyst = " << RunSyst << endl;
...
//  MuonIDSFKeys = { "NUM_TightID_DEN_genTracks" };
  ...
  //==== I defined "TString IsoMuTriggerName;" and "double TriggerSafePtCut;" in Analyzers/include/HNtypeI_WZ_CR.h 
  ...
  cout << "[HNtypeI_WZ_CR::initializeAnalyzer] IsoMuTriggerName = " << IsoMuTriggerName << endl;
  cout << "[HNtypeI_WZ_CR::initializeAnalyzer TriggerSafePtCut = " << TriggerSafePtCut << endl;
...
void HNtypeI_WZ_CR::executeEvent(){
  ...
  //==== So, I defined "vector<Muon> AllMuons;" in Analyzers/include/HNtypeI_WZ_CR.h,
  ...
  //==== I defined "double weight_Prefire;" in Analyzers/include/HNtypeI_WZ_CR.h
...
void HNtypeI_WZ_CR::executeEventFromParameter(AnalyzerParameter param){
...
//  if(! (ev.PassTrigger(IsoMuTriggerName) )) return;
  ...
  if(param.syst_ == AnalyzerParameter::Central){
    ...
    cout << "[HNtypeI_WZ_CR::executeEventFromParameter] Wrong syst" << endl;
    exit(EXIT_FAILURE);
  }
...
//    if(IsBTagged(jets.at(ij), Jet::DeepCSV, Jet::Medium,false,0)) n_bjet_deepcsv_m_noSF++; // method for getting btag with no SF applied to MC
  ...
  if(lepton_size == 3){
    ...
    if(MET < 50.) return;
    double Mt = MT(WtagLep, METv);
    if(Mt < 20.) return;
    if(TriLep.M() < 91.1876 + 15.) return;
    if(n_bjet_csvv2 > 0) return;
    ...
    FillHist("ZCand_Mass", ZCand.M(), weight, 80, 50., 130.);
    FillHist("WtagLep_Pt", WtagLep.Pt(), weight, 400, 0., 400.);
    FillHist("ZtagLep1_Pt", ZtagLep1.Pt(), weight, 400, 0., 400.);
    FillHist("ZtagLep2_Pt", ZtagLep2.Pt(), weight, 400, 0., 400.);
    FillHist("MET", MET, weight, 400, 0., 400.);
    FillHist("Mt", Mt, weight, 400, 0., 400.);
  }
}
