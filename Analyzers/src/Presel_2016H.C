...
//          ele_idsf = mcCorr->ElectronID_SF(param.Electron_ID_SF_Key, electrons.at(j).scEta(), electrons.at(j).Pt(), 0); //JH : FIXME no electron ID SF applied
      ...
      if(IsDATA && leptons.at(0)->Charge()*leptons.at(1)->Charge()>0) continue;

      // Cutflow : same-sign (oppsite-sign when OS or data)
      ...
      if(fatjets.size()>0) FillHist(channels.at(it_ch)+"/Pre/FatJet_Pt_"+IDsuffix, fatjets.at(0).Pt(), weight, 1000, 0., 1000.);
      if(fatjets.size()>0) FillHist(channels.at(it_ch)+"/Pre/FatJet_Eta_"+IDsuffix, fatjets.at(0).Eta(), weight, 54, -2.7, 2.7);
      if(jets.size()>0){
