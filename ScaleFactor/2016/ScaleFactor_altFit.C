...
void ScaleFactor(TString id, TString EtaRegion, int Syst = 0, TString SaveAs = "n"){ // BB, BE, EE
  ...
  TString filename = "/data4/Users/jihkim/SKFlatOutput/Run2Legacy_v3/ChargeFlip/2016/ScaleFactor__RunSyst__/DATA/ChargeFlip_DoubleEG_total_NBin40_DYTTLLCF.root";
  TFile* f1 = new TFile(filename);
  
  TString samplename = "altFit"+filename(filename.Last('_')+1,filename.Length());
  ...
  fitFcn_tmp->SetParameters(0,0,70,90,0.5); // Parameters to initialize fit function
  ...
  vector<double> y_4;
  for (int i=0; i<NBin; i++) {
    y_4.push_back(gaussianPeak(&x_4[i],&par[2])/h1->GetBinContent(i+1));
  }
  ...
  gr3->GetYaxis()->SetRangeUser(0,1.4);
  gPad->SetGridx();
  gPad->SetGridy();
