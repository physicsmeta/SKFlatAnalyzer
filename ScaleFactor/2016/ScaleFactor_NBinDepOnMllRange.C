...
void ScaleFactor_tmp(TString id, TString EtaRegion, int Syst = 0, TString SaveAs = "n"){ // BB, BE, EE
  ...
  int NBin = 40; if(Syst==3||Syst==4) NBin = MllRight-MllLeft; else if(Syst==5) NBin = 35; else if(Syst==6) NBin = 45;
  double err = 0.5; if(Syst==5||Syst==6) err = 1.*(MllRight-MllLeft)/NBin/2.; // if NBin depends on MllRange, then err changes only for Syst 5 and 6
  ...
  TString filename = "/data4/Users/jihkim/SKFlatOutput/Run2Legacy_v3/ChargeFlip/2016/ScaleFactor__RunSyst__/DATA/ChargeFlip_DoubleEG_total_NBinDepOnMllRange_DYTTLLCF.root";
  TFile* f1 = new TFile(filename);
  
  TString samplename = filename(filename.Last('_')+1,filename.Length());
  samplename.ReplaceAll(".root","");
  
  gSystem->Exec("mkdir NBinDepOnMllRange_"+samplename);
  ...
  if(SaveAs == "y") {
    ...
    c1->SaveAs("NBinDepOnMllRange_"+samplename+"/"+id+"_SS2lFit_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+".pdf");
    c2->SaveAs("NBinDepOnMllRange_"+samplename+"/"+id+"_ScaleFactor_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+".pdf");
  
    c1->SaveAs("NBinDepOnMllRange_"+samplename+"/"+id+"_SS2lFit_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+".png");
    c2->SaveAs("NBinDepOnMllRange_"+samplename+"/"+id+"_ScaleFactor_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+".png");
